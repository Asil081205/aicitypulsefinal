# 🌆 AI City Pulse – Digital Health Monitor for Cities

AI City Pulse is an innovative smart city mobile application that treats a city like a living organism and measures its "health" similar to a human pulse.

This project supports **United Nations SDG 11 – Sustainable Cities and Communities** by using Artificial Intelligence to monitor, analyze, and visualize city health indicators in real time.

---

## 🚀 Project Vision

Cities generate massive amounts of data every second:
- 🚦 Traffic congestion
- 🌫️ Air pollution
- 🔊 Noise levels
- 👥 Crowd density
- ⚡ Power usage

AI City Pulse converts this data into a single **City Stress Score** to determine whether a city is healthy, overloaded, or at risk.

---

## 🧠 Core Concept

Just like a doctor checks:
- Heart Rate
- Blood Pressure
- Oxygen Level

AI City Pulse checks:
- Traffic Pressure
- Pollution Index
- Noise Index
- Crowd Density
- Energy Consumption

Then calculates a:

# 💓 City Health Score

---

## 📱 Key Features

### 📊 1. City Health Dashboard
- Real-time health score
- Visual stress meter
- AI-based recommendations
- Historical trend graphs

### 🗺️ 2. Smart City Map
- Heatmap of overloaded zones
- Green (Healthy) / Yellow (Moderate) / Red (Critical) areas
- Tap-to-view zone details

### 🤖 3. AI Prediction Engine
- Predicts congestion risk
- Predicts pollution spikes
- Emergency risk alerts

### 📈 4. City Analytics
- Data visualization charts
- Stress comparison between zones
- Smart recommendations

### 🔔 5. Alert System
- High pollution alerts
- Traffic overload warnings
- Emergency notifications

---

## 🏗️ Tech Stack

### 📱 Frontend
- Flutter (Dart)
- Material UI
- Google Maps API

### 🧠 Backend / AI
- Python
- Machine Learning Models
- Data Analysis
- REST API integration

### 📊 Data Sources (Simulated / Real-time capable)
- Traffic datasets
- Pollution APIs
- Smart city IoT simulation
- Crowd density estimation

---

## 🏛️ Architecture

User Interface (Flutter) ↓ View Model Layer ↓ City Repository ↓ API Service ↓ AI Processing Engine ↓ Data Storage / External APIs

This modular architecture ensures:
- Scalability
- Maintainability
- Clean code structure

---

## 📸 Screenshots

![WhatsApp Image 2026-02-13 at 2 05 30 AM](https://github.com/user-attachments/assets/bfaa92ed-7223-4d0d-b00d-3f73a218e267)

![WhatsApp Image 2026-02-13 at 2 05 31 AM](https://github.com/user-attachments/assets/8028f209-41d0-4dc8-9f20-eecc6a2c361b)

![WhatsApp Image 2026-02-13 at 2 05 31 AM (1)](https://github.com/user-attachments/assets/63a164b9-de6e-46fc-a362-ccb48de454cf)

![WhatsApp Image 2026-02-13 at 2 05 31 AM (2)](https://github.com/user-attachments/assets/fc695aaa-836b-4f8f-86c6-94b9c292ab09)

![WhatsApp Image 2026-02-13 at 2 05 32 AM](https://github.com/user-attachments/assets/8a0d2c95-7695-4a30-9b39-7a52b380d800)

![WhatsApp Image 2026-02-13 at 2 05 32 AM (1)](https://github.com/user-attachments/assets/0a5ae94c-7ad7-4dbe-94a4-6dbb55d9f434)

![WhatsApp Image 2026-02-13 at 2 05 33 AM](https://github.com/user-attachments/assets/9be9583c-186f-46e0-9e56-bd5d955357f3)

![WhatsApp Image 2026-02-13 at 2 05 33 AM (1)](https://github.com/user-attachments/assets/0b992d84-142e-4ef5-a7bc-45b41dfb30e0)

---

## 🛠️ Installation Guide

1️⃣ Clone the Repository

git clone https://github.com/Asil081205/aicitypulsefinal.git

2️⃣ Navigate to Project

cd aicitypulsefinal

3️⃣ Install Dependencies

flutter pub get

4️⃣ Run the App

flutter run

##🎯 Use Cases

Smart City Planning

Urban Policy Making

Disaster Prevention

Environmental Monitoring

Sustainable Development Research

##🌍 SDG Impact

AI City Pulse directly contributes to:

✔ SDG 11 – Sustainable Cities
✔ SDG 13 – Climate Action
✔ SDG 9 – Industry, Innovation & Infrastructure

##🔮 Future Enhancements

Live IoT integration

Satellite data analysis

AI-powered anomaly detection

Citizen feedback system

Drone-based monitoring

Blockchain for data transparency

##👨‍💻 Developer

Asil Jamesha T
B.Tech – Artificial Intelligence & Data Science
Aalim Muhammed Salegh College of Engineering
CGPA: 8.24

🔗 GitHub: https://github.com/Asil081205

🔗 LinkedIn: https://linkedin.com/in/asil-jamesha-t-988995336

📧 Email: asilansari08@gmail.com

##📜 License

This project is developed for educational and research purposes.
Open for collaboration and innovation.


